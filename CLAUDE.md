...
- "Check connections with AI" (`spec_ai.py::review_connections`, `/review-connections[-stream]`) reuses the exact same findings format and fix-op vocabulary, so the same CheckPanel/preview/apply machinery serves both. AI proposals are sanitized server-side (`_sanitize_op`: only real component/part names, nudges ≤ 0.5 m, known connection types) and the fixed spec is test-built before the response ships.
