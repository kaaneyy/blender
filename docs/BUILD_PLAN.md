...
- [x] E3.1 Hardware v2 — engineered joints: through-bolt assemblies whose shafts span the actual joint (+25 mm embedment) with washers under hex head and nut, bolt diameter scaled to the joint face, 2-/4-bolt patterns with real edge distances on wide faces, saddle band clamps (taper-following radius + 2 side bolts) where horizontal round members meet upright poles, oriented bounding boxes for rotated cylinders (no phantom bolts), too-thin-to-drill faces skipped. LLM system prompt gained CONNECTION RULES (parts must interpenetrate 10-20 mm, model rails/brackets/base plates, real load paths); the install guide now enumerates every joint with fastener type and on-site execution; the park bench few-shot gained frame rails so slats genuinely bolt to structure.
